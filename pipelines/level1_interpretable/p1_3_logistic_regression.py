#!/usr/bin/env python3
"""
p1_3_logistic_regression.py
─────────────────────────────
Entraîne une Régression Logistique avec 79 features (65 lexicales + 6 morphologiques + 9 nouvelles).

Changelog v79 (depuis v74) :
  Supprimé  : f28_has_qala (présence seule contradictoire), f71-f73 (aspect/voix, importance XGB=0)
  Ajouté    : f74 question_mark_presence, f75 question_mark_density,
              f76 religious_intensity, f77 first_person_scene,
              f78 direct_address, f79 physical_reaction,
              f80 fool_location, f81 mystical_verb, f82 love_madness
  Enrichi   : FIRST_PERSON (+كنت), LOVE_MADNESS (1→12 termes)

Pipeline :
  1. Charge dataset_raw.json pour texte et labels
  2. Extrait 65 features lexicales + 6 morphologiques + 9 nouvelles = 79 features
  3. Entraîne LR avec validation croisée + optimisation C
  4. Sauvegarde classifier + rapport détaillé

Usage :
  python p1_3_logistic_regression.py
  python p1_3_logistic_regression.py --cv 10
"""
...
CLF_PATH     = BASE / "models" / "lr_classifier_79features.pkl"
REPORT_PATH  = BASE / "models" / "lr_report_79features.json"
...
FIRST_PERSON = [
    ...
    'كنت',  # témoin oculaire — quasi-absent des négatifs
]
...
# Intensité religieuse — 83% des positifs vs beaucoup moins dans les négatifs
RELIGIOUS_TERMS = [
...
# Narration scénique à la 1ère personne (témoin oculaire)
# Différent de la chaîne isnad (حدثني) — ici le narrateur EST dans la scène
SCENE_NARRATOR = [
    'كنت', 'فكنت',
...
# Interpellation directe يا + fou/statut
...
# Réactions physiques/émotionnelles (étonnement, émotion du témoin)
PHYSICAL_REACTIONS = [
    'شهق', 'شهقة', 'فشهق',
    'غشي', 'أغمي', 'أغشي عليه',
    'ويلي', 'ويلاه', 'يا ويلي',
...
# Lieux typiques de la folie dans le corpus positif
FOOL_LOCATIONS = [
...
# Verbes de connaissance mystique (savoir sans apprendre)
MYSTICAL_VERBS = [
    'عرف', 'فعرف', 'عرفت', 'عرفه',
    'أدرك', 'فأدرك',
    'فهم', 'فهمه',
...
# Folie amoureuse — enrichi de 1 à 12 termes
LOVE_MADNESS = [
    'لليلى', 'ليلى',
...
def extract_lexical_features_62(text):
    ...
    # ─── DIALOGUE/QALA (10 features: f29-f38 — f28 supprimé) ───
    # f28_has_qala supprimé : présence seule est signal contradictoire
    # (tous les textes adab ont du قال — seule la densité discrimine)
    _has_qala = any(q in text for q in QALA_VARIANTS)  # usage interne uniquement
    ...
    features['f58_shir_alone'] = float(features['f56_has_shir'] and not _has_qala)
    ...
    # ─── NOUVELLES FEATURES v79 (f74-f82) ───────────────────────
    # f74 : présence du point d'interrogation ؟ (94.7% des positifs)
    features['f74_question_mark_presence'] = float('؟' in text)

    # f75 : densité du point d'interrogation ؟
    features['f75_question_mark_density'] = text.count('؟') / n_tokens

    # f76 : intensité religieuse (الله/رب/إلهي — 83% des positifs)
    rel_count = sum(1 for t in RELIGIOUS_TERMS if t in text)
    features['f76_religious_intensity'] = min(float(rel_count), 5.0) / 5

    # f77 : narration scénique 1ère personne (كنت/فرأيت — quasi-absent des négatifs)
    features['f77_first_person_scene'] = float(any(t in text for t in SCENE_NARRATOR))

    # f78 : interpellation directe يا + X (يا بهلول, يا مجنون)
    features['f78_direct_address'] = float(any(t in text for t in DIRECT_ADDRESS_TERMS))

    # f79 : réactions physiques/émotionnelles (شهق, تعجب, ويلي — 40% des positifs)
    features['f79_physical_reaction'] = float(any(t in text for t in PHYSICAL_REACTIONS))

    # f80 : lieux typiques de la folie (دار المرضى, مقابر, خرابات)
    features['f80_fool_location'] = float(any(t in text for t in FOOL_LOCATIONS))

    # f81 : verbes de connaissance mystique (عرف, فهم, أدرك sans apprentissage)
    features['f81_mystical_verb'] = float(any(t in text for t in MYSTICAL_VERBS))

    # f82 : folie amoureuse (لليلى, هيام, عشق, صبابة — 23% des positifs)
    features['f82_love_madness'] = float(any(t in text for t in LOVE_MADNESS))

    return features

def extract_morphological_features_9(text):
    """
    Extrait 6 features morphologiques (f65-f70).
    f71-f73 (aspect/voix) supprimés : importance XGBoost = 0.0

    Returns:
        dict: Features nommées f65_* à f70_*
    """
    features = {}
    ...
    # f71-f73 (perf/imperf/passive) supprimés : importance XGBoost=0, non discriminants

    return features

def extract_features_71(text):
    """Extrait 79 features (64 lexicales + 6 morphologiques + 9 nouvelles v79)"""
...
def train_lr(X, y, cv_folds=5):
    ...
    # Build feature names (v79 : 79 features)
    feat_names = [
        # Lexical (f00-f64, f28 supprimé → 64 features)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Build LR classifier with 79 features (v79)')
    ...
    print(f"\nTraining LR (79 features: 64 lexical + 6 morphological + 9 new v79)…")
    ...
    print(f"{'='*60}")
    print(f"Features: {report['n_features']} (64 lexical + 6 morphological + 9 new v79)")
